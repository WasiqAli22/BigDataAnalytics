...
app.get('/timers', (req, res) => {
    if (fileTimingStats.length === 0) {
        return res.send('<html><body><h1>No timing data yet</h1></body></html>');
    }

    const totalFiles = fileTimingStats.length;
    const avgTotal = fileTimingStats.reduce((acc, f) => acc + f.totalTime, 0) / totalFiles;
    const avgMatch = fileTimingStats.reduce((acc, f) => acc + f.matchTime, 0) / totalFiles;
    ...
    fileTimingStats.slice(-100).forEach(stat => {
        page += `<tr>
            <td>${stat.filename}</td>
            <td>${stat.lineCount}</td>
            <td>${stat.totalTime.toFixed(2)}</td>
            <td>${stat.matchTime.toFixed(2)}</td>
            <td>${stat.timestamp.toLocaleTimeString()}</td>
        </tr>`;
    });
...
var lastFile = null;
//Creating a global array to store timing stats
const fileTimingStats = [];
    ...
    Promise.resolve({ name: filename, contents: contents })
      ...
      .then(
        PASS((file) => {
          ...
          fileTimingStats.push({
            filename: file.name,
            totalTime: Number(timers["total"]) / 1000, // microseconds
            matchTime: Number(timers["match"]) / 1000,
            lineCount: file.lines ? file.lines.length : 0,
            timestamp: new Date(),
          });

          // only keep last N entries to avoid memory growth
          if (fileTimingStats.length > 1000) {
            fileTimingStats.shift();
          }
        })
      )
